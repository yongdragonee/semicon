...
# ───────────────────────────────────────────────
# 1) Chrome 지문 세션 만들기  ─────────────────────────────────
YF_SESS = requests.Session(impersonate="chrome")

# 2) 헬퍼 함수 : 모든 티커를 한 번에 받고 DataFrame 반환  ───
@st.cache_data(ttl=300)                 # 같은 날에는 캐시
def download_prices(ticker_list, start, end):
    """yfinance 우회 다운로드 (curl_cffi 세션 사용)"""
    # 공백 구분 문자열이 yfinance가 가장 덜 막히는 형식
    tickers = " ".join(ticker_list)
    df = yf.download(
        tickers,
        start=start, end=end,
        session=YF_SESS,                 # ← 브라우저 지문 세션 주입
        progress=False, threads=False    # 병렬 요청 OFF
    )
    # yfinance는 여러 티커일 때 MultiIndex 컬럼을 만든다.
    # 이 스크립트는 'Close' 만 쓰므로 한 번에 꺼내 줌
...
st.sidebar.write("3/5 Streamlit 오픈 · 3/13 검색 추가 · 3/29 주가현황 추가")
